...
import upload from "../controllers/uploadFile/uploadFileExcel";
import exportAssetReport from "../controllers/uploadFile/exportFile"
import crudPosition from "../controllers/position/crudPosition";
import token from "../middlewares/jwt.middleware";
import schedule from "node-schedule";

const router = Router();
router.route("/auth/login").post(token.verifyToken, validate(validation.login), login); //route đăng nhập
...
//User
router.route("/user").get(token.verifyToken, crudUser.getUserData); //route trả về tất cả user
router.route("/user").post(token.verifyToken, token.checkAdmin, validate(validation.createUser), crudUser.create); //route tạo tài khoản
