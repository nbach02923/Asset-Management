...
export class Nodejs1695435119957 implements MigrationInterface {
    name = 'Nodejs1695435119957'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`asset\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`serial\` varchar(255) NOT NULL, \`type\` enum ('Stationary', 'Nonstationary', 'Other') NOT NULL, \`status\` enum ('Ready to Deploy', 'Deployed', 'Error') NOT NULL, \`description\` text NULL, \`warrantDate\` date NULL, \`buyDate\` date NULL, \`categoryAssetId\` varchar(255) NOT NULL, \`isDeleted\` tinyint NOT NULL DEFAULT 0, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_97722584aab48508512c63aae5\` (\`serial\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
